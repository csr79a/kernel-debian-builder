...
6. Descarga el código fuente y verifica su integridad: suma **SHA256** y **firma PGP** contra las claves oficiales de kernel.org.
7. Genera la configuración (`.config`) a partir del kernel que tienes corriendo actualmente.
8. Fuerza las opciones de hardware ASUS (`CONFIG_ASUS_ARMOURY` y afines, necesarias para `asusctl`/ROG Control Center) y de sched-ext (`CONFIG_SCHED_CLASS_EXT` y sus dependencias BPF/BTF) — ver el MANUAL para el porqué.
9. Compila el kernel y genera paquetes `.deb` (`linux-image`, `linux-headers`) usando el método nativo del propio kernel (`make bindeb-pkg`).
10. Instala los paquetes con `apt install` (resuelve dependencias automáticamente).
11. Regenera GRUB (`update-grub`).
12. Pregunta si quieres eliminar los archivos generados durante la compilación.
13. Pantalla final con opción de reiniciar ahora o después.
...
Si ya compilaste un kernel con este script y quieres recompilar la misma versión (por ejemplo, tras cambiar una opción forzada en el script), usa `--force`:
...
- Fuerza opciones de hardware ASUS y de sched-ext que Debian no trae activadas en su kernel de serie — ver "Forzar opciones..." en el MANUAL.
